...
float overlap(vec2 a,vec2 b){
  ...
  float right=min(a.x+1.,b.x+1.);
  float bottom=max(a.y,b.y);
  float top=min(a.y+1.,b.y+1.);
  return max(1.,(right-left)*(top-bottom));
}
...
void main(){
  ...
  d=clamp(d-d*abs(v.y),0.,1.);
  v.y-=abs(v.y*v.y);
  
  // Factor incoming speed and density
  vec4 s=sample(t+vec2(0.,-1.));
  if(s.y>0.){
    d+=s.z*abs(s.y);
    v.y+=s.y*abs(s.y);
  }
  
  vec4 n=sample(t+vec2(0.,1.));
  if(n.y<0.){
    d+=n.z*abs(n.y);
    v.y+=n.y*abs(n.y);
  }
  ...
  // Apply forces
  v.y=clamp(v.y-u_gravity*u_delta,-1.,1.);
  ...
  gl_FragColor=vec4(vec3(v.x,v.y,d),1.);
